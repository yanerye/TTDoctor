...
#define SCREENWIDTH(x)   CGRectGetWidth([UIScreen mainScreen].bounds) * x / 320
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Initialization code
        self.borderColor = RGBACOLOR(200, 199, 204);
...
-(CGRect)getViewFrame
{
    ...
    frame.size.height = [self.titleArray count] * ROW_HEIGHT + SPACE + kArrowHeight + 2;
    
    for (NSString *title in self.titleArray) {
        CGFloat width = [title boundingRectWithSize:CGSizeMake(300, 100) options:NSStringDrawingUsesLineFragmentOrigin |   NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName: TITLE_FONT}  context:nil].size.width;
    ...
    //右间隔最小5x
    if ((frame.origin.x + frame.size.width) > SCREENWIDTH(315)) {
        frame.origin.x = SCREENWIDTH(315) - frame.size.width;
    }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.backgroundView.backgroundColor = _bgColor;
    ...
    cell.textLabel.textColor = UIColorFromRGB(0x0084d0);
