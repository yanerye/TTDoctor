...
@interface YKMainSymptomVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation YKMainSymptomVC
{
    YKDoctor *_doctor;
    NSString *_deptName;
...
- (void)getData{
    ...
    [[YKApiService service] getDiseasesTypeListCompletion:^(id responseObject, NSError *error) {
        if (!error) {
...
- (void)doneClick{
    ...
    [[YKApiService service] saveskilledDiseasesWithGoodsIDs:goodsIDString completion:^(id responseObject, NSError *error) {
        if (!error) {
            [YKHUDHelper hideHUDInView:self.view];
